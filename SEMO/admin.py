...
async def admin_risght(client: Client, message):
  try:
    ...
    if command == "/pause" or command == "ايقاف مؤقت":
       ...
       button = [[InlineKeyboardButton(text="𝐄𝐍𝐃", callback_data=f"stop"), InlineKeyboardButton(text="𝐑𝐄𝐒𝐔𝐌𝐄", callback_data=f"𝐑𝐄𝐒𝐔𝐌𝐄"), InlineKeyboardButton(text="𝐏𝐀𝐔𝐒𝐄", callback_data=f"𝐏𝐀𝐔𝐒𝐄")], [InlineKeyboardButton(text="𝗖𝗵𝗮𝗻𝗻𝗲𝗹 ", url=f"{ch}"), InlineKeyboardButton(text="𝗚𝗿𝗼𝘂𝗽 ", url=f"{gr}")], [InlineKeyboardButton(text=f"{OWNER_NAME}", url="https://t.me/C6_6V1")], [InlineKeyboardButton(text="اضف البوت الي مجموعتك او قناتك ⚡", url=f"https://t.me/{bot_username}?startgroup=True")]]
